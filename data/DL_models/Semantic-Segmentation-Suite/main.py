...
parser.add_argument('--is_training', type=str2bool, default=True, help='Whether to use training mode of batch norm')
...
if args.model == "FC-DenseNet56" or args.model == "FC-DenseNet67" or args.model == "FC-DenseNet103":
    ...
    network, init_fn = build_deeplabv3_plus(net_input, preset_model = args.model, num_classes=num_classes, is_training=args.is_training)
...
if args.class_balancing:
    ...
    elif args.loss_func == "sigmoid":
        unweighted_loss = tf.nn.sigmoid_cross_entropy_with_logits(logits=network, labels=net_output)
...
if args.mode == "train":
    ...
    class_names_list, label_values = helpers.get_label_info(os.path.join("Corine-Sentinel-DEM", "class_dict.csv"))
    ...
    # Create directories if needed
    if not os.path.isdir("%s"%("TEST7s")):
            os.makedirs("%s"%("TEST7s"))

    target=open("%s/test_scores.csv"%("TEST7s"),'w')
    ...
    for ind in range(len(test_input_names)):
        ...
        cv2.imwrite("%s/%s_pred.png"%("TEST7s", file_name),cv2.cvtColor(np.uint8(out_vis_image), cv2.COLOR_RGB2BGR))
        cv2.imwrite("%s/%s_gt.png"%("TEST7s", file_name),cv2.cvtColor(np.uint8(gt), cv2.COLOR_RGB2BGR))
